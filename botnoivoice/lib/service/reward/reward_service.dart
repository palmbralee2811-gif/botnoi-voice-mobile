...
class RewardService {
    ...
    try {
      ...
      if (couponCode == null) {
        ...
        if (_ref.read(errorMessageProvider) == null) {
          _ref.read(errorMessageProvider.notifier).state = 'reward_service.no_daily_coupon_to_redeem'.tr();
          _logger.w('checkCoupon100: No daily coupon code available to redeem (${_ref.read(errorMessageProvider)})');
        } else {
          _logger.e('checkCoupon100: Error occurred while getting coupon name: ${_ref.read(errorMessageProvider)}');
        }
        return;
      }
      // ---------------
  ...
  Future<void> getEducationSubscription(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (response.statusCode == 403) {
          if (response.body.contains('already have subscription')) {
            newErrorMessage = 'reward_service.already_subscribed'.tr(); // Already subscribed
          }
          else if (response.body.contains('invalid sign in provider')) {
            newErrorMessage = 'reward_service.failure_provider'.tr(); // Invalid provider
          }
          else if (response.body.contains('invalid domain adn whitelist education')) {
            newErrorMessage = 'reward_service.failcase_whitelist'.tr(); // Not whitelisted
          }
          else {
            newErrorMessage = 'reward_service.failure_case'.tr(); // General failure case
          }
        } else {
          // For other errors
          final responseBody = jsonDecode(response.body);
  ...
  Future<String?> _fetchJwtToken(BuildContext context) async {
    try {
      ...
      if (jwtToken == null) {
        ...
        _ref.read(errorMessageProvider.notifier).state = 'Failed to fetch ID token';
  ...
  Future<String?> _getCouponNameForToday() async {
    _setLoading(true);
    String? localErrorMessageForThisCall; // Local variable for this call's error

    try {
      _logger.d('Fetching daily coupon name from $apiUrl/api/coupon/get_coupon_daily');
      final response = await http.get(Uri.parse('$apiUrl/api/coupon/get_coupon_daily'));
      ...
      if (response.statusCode == 200) {
        ...
        if (couponName != null && couponName.isNotEmpty) {
          _logger.i('Daily coupon name for today: $couponName');
          _ref.read(errorMessageProvider.notifier).state = null; // Clear error on success
          return couponName;
        } else {
          _logger.i('No daily coupon name found (API returned 200 OK but no/empty name). This is treated as "no coupon data".');
          _ref.read(errorMessageProvider.notifier).state = null;
          return null;
        }
      } else if (response.statusCode == 404) {
        _logger.i('No daily coupon set for today (API returned 404 Not Found). This is a valid "no coupon" state.');
        _ref.read(errorMessageProvider.notifier).state = null;
        return null;
      } else {
        localErrorMessageForThisCall =
            'Failed to fetch daily coupon name: ${response.statusCode} - ${response.body.substring(0, (response.body.length > 150) ? 150 : response.body.length)}';
        _logger.e(localErrorMessageForThisCall);
        _ref.read(errorMessageProvider.notifier).state = localErrorMessageForThisCall; // Set as Service error
        return null;
      }
    } catch (e) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (message == 'Use Coupon Success') {
          ...
          _ref.read(errorMessageProvider.notifier).state = 'reward_service.coupon_already_used'.tr();
          _logger.e(_ref.read(errorMessageProvider));
        } else if (message == 'Incorrect Coupon') {
          // Coupon not found or no longer usable.
          _ref.read(errorMessageProvider.notifier).state = 'reward_service.coupon_not_found'.tr();
          _logger.e(_ref.read(errorMessageProvider));
        } else {
          // Coupon not found in the system or has expired.
          _ref.read(errorMessageProvider.notifier).state = 'reward_service.coupon_expired_or_not_found'.tr();
          _logger.e(_ref.read(errorMessageProvider));
        }
      } else {
        // Error occurred during API call.
        final errorMessage = '${'reward_service.api_call_error'.tr()} ${response.statusCode}';
        _ref.read(errorMessageProvider.notifier).state = errorMessage;
        _logger.e(errorMessage);
      }
    } catch (e) {
