...
/// Google: Token, User Profile Data and Credentials using Riverpod StateNotifier.
class GoogleTokenNotifier extends StateNotifier<UserTokenState> {
  final Ref _ref;
  final Logger _logger = Logger(); // For debugging

  GoogleTokenNotifier(this._ref) : super(UserTokenState());
  ...
  Future<void> loadJwtToken() async {
    // Get the GoogleLoginNotifier instance via ref
    final googleLoginNotifier = _ref.read(googleLoginNotifierProvider.notifier);
    // Get the idToken from the Authentication provider
    String? idToken = await googleLoginNotifier.user?.getIdToken();
...
/// Riverpod provider for GoogleTokenNotifier.
final googleTokenNotifierProvider = StateNotifierProvider<GoogleTokenNotifier, UserTokenState>((ref) {
  return GoogleTokenNotifier(ref);
});