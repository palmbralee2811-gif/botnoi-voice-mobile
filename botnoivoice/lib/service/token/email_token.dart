...
//     String? idToken = await context.read<EmailLogin>().user?.getIdToken();
//     if (idToken == null) {
...
/// Email: Token, User Profile Data and Credentials using Riverpod StateNotifier.
class EmailTokenNotifier extends StateNotifier<UserTokenState> {
  final Ref _ref;
  final Logger _logger = Logger(); // For debugging

  EmailTokenNotifier(this._ref) : super(UserTokenState());
  ...
  Future<void> loadJwtToken() async {
    // Get the EmailLoginNotifier instance via ref
    final emailLoginNotifier = _ref.read(emailLoginNotifierProvider.notifier);
    // Get the idToken from the Authentication provider
    String? idToken = await emailLoginNotifier.user?.getIdToken();
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['data'] != null && data['data']['token'] != null) {
          state = state.copyWith(jwtToken: data['data']['token']);
          _logger.i('JWT Token successfully loaded.');
        } else {
          _logger.w('Token not found in response data.');
        }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        state = state.copyWith(
          ...
          // Check ว่าผู้ใช้ได้ Subscription ยัง
...
/// Riverpod provider for EmailTokenNotifier.
final emailTokenNotifierProvider = StateNotifierProvider<EmailTokenNotifier, UserTokenState>((ref) {
  return EmailTokenNotifier(ref);
});