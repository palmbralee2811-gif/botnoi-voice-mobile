...
abstract class UserLoginBaseNotifier<T extends UserLoginBaseState> extends StateNotifier<T> {
  ...
  UserLoginBaseNotifier(super.initialState, this._providerId) {
    ...
    FirebaseAuth.instance.authStateChanges().listen((User? user) {
      if (user?.providerData.isNotEmpty == true && 
        ...
        _updateState(user: user, isLoggedIn: true, errorMessage: null);
      } else {
        ...
        if (state.isLoggedIn && user == null) {
          ...
          _updateState(user: null, isLoggedIn: false, errorMessage: null); // Reset state on logout
        }
      }
    });
  }
