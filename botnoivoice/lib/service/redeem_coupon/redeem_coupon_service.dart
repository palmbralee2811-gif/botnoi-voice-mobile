...
class RedeemCouponService with ChangeNotifier {
  ...
  Future<String?> _callCheckCouponApi(String jwtToken, String couponName) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (message.toString().toLowerCase() == 'use coupon success') {
          ...
          errorMessage = 'redeem_coupon_service.coupon_already_used'.tr(namedArgs: {'coupon_name': couponName});
          _logger.e(errorMessage);
          return errorMessage;
          ...
          errorMessage = 'redeem_coupon_service.coupon_not_found'.tr(namedArgs: {'coupon_name': couponName});
          _logger.e(errorMessage);

          return errorMessage;
        } else {
          //ไม่พบคูปองในระบบ หรือคูปองหมดอายุไปแล้ว
          errorMessage = 'redeem_coupon_service.coupon_expired_or_not_found'.tr(namedArgs: {'coupon_name': couponName});
          _logger.e(errorMessage);
        ...
        errorMessage = '${'redeem_coupon_service.api_call_error'.tr()} ${response.statusCode}';
        _logger.e(errorMessage);
  ...
  Future<String?> redeemCoupon(BuildContext context, String couponCode) async {
    _setLoading(true);
    try {
