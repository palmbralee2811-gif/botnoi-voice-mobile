  ...
  Future<void> loginWithUsernamePassword(
    ...
    try {
      ...
      if (email == 'email not found') {
        _errorMessage = 'login_provider.invalid_username'.tr(); //ชื่อผู้ใช้งานไม่ถูกต้อง
      ...
      _errorMessage = "${'login_provider.error_logging_in'.tr()} $e"; //เกิดข้อผิดพลาดในการเข้าสู่ระบบ:
  ...
  Future<void> loginWithEmailPassword(String email, String password) async {
    try {
      ...
      if (!userCredential.user!.emailVerified) {
        _errorMessage = 'login_provider.please_verify_email_before_login'.tr(); //กรุณายืนยันอีเมลก่อนเข้าสู่ระบบ
      ...
      switch (e.code) {
        case 'invalid-email':
          ...
          break;
        case 'wrong-password':
          _errorMessage = 'login_provider.incorrect_password'.tr(); //รหัสผ่านไม่ถูกต้อง
          break;
        case 'user-disabled':
          _errorMessage = 'login_provider.account_suspended'.tr(); //บัญชีนี้ถูกระงับการใช้งาน
          break;
        default:
          _errorMessage = 'login_provider.username_or_password_incorrect'.tr(); //ชื่อผู้ใช้งานหรือรหัสผ่านไม่ถูกต้อง
          break;
      }
      _logger.e(
