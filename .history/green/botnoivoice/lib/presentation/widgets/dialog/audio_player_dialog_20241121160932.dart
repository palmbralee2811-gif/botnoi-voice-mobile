...
class _AudioPlayerDialogState extends State<AudioPlayerDialog> {
  ...
  Future<void> _checkAndroidRequestPermissions() async {
    ...
    if (!hasPermission) {
      AndroidPermissionDialog(
              context: context, text: 'app_drawer.permission_denied'.tr()) //สิทธิ์ถูกปฏิเสธ กรุณาไปที่การตั้งค่า
  ...
  Future<void> _initAudioPlayer() async {
    try {
      ...
      if (!await file.exists() || await file.length() == 0) {
        throw Exception('app_drawer.audio_file_not_found'.tr()); //ไม่พบไฟล์เสียงหรือไฟล์ว่างเปล่า
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("${'app_drawer.error_playing_audio'.tr()} $e")), //เกิดข้อผิดพลาดในการเล่นไฟล์เสียง:
        );
      }
    }
  }
  ...
  Future<void> _downloadFileToCustomPath() async {
    ...
    if (isSaved == false) {
      NotificationPopup(context: context, text: 'app_drawer.unable_to_save_file'.tr()) //ไม่สามารถบันทึกไฟล์ได้
          .showAsError();
    }
  }
      ...
      child: Padding(
        ...
        child: isLoading
            ...
            : Column(
                ...
                children: [
                  Text(
                    'app_drawer.audio_created_successfully'.tr(), //สร้างเสียงสำเร็จ
                  ...
                  GradientRow(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        SizedBox(width: 8.w),
                        Text(
                          'app_drawer.download'.tr(), //ดาวน์โหลด
                          style: GoogleFonts.prompt(
                            color: Colors.white,
                            fontSize: 16.sp,
