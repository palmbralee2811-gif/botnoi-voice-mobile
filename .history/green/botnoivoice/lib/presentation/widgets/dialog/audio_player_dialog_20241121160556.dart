...
class _AudioPlayerDialogState extends State<AudioPlayerDialog> {
  ...
  Future<void> _checkAndroidRequestPermissions() async {
    ...
    if (!hasPermission) {
      AndroidPermissionDialog(
              context: context, text: "สิทธิ์ถูกปฏิเสธ กรุณาไปที่การตั้งค่า") //สิทธิ์ถูกปฏิเสธ กรุณาไปที่การตั้งค่า
  ...
  Future<void> _initAudioPlayer() async {
    try {
      ...
      if (!await file.exists() || await file.length() == 0) {
        throw Exception("ไม่พบไฟล์เสียงหรือไฟล์ว่างเปล่า"); //ไม่พบไฟล์เสียงหรือไฟล์ว่างเปล่า
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("เกิดข้อผิดพลาดในการเล่นไฟล์เสียง: $e")), //เกิดข้อผิดพลาดในการเล่นไฟล์เสียง:
        );
      }
    }
  }
  ...
  Future<void> _downloadFileToCustomPath() async {
    ...
    if (isSaved == false) {
      NotificationPopup(context: context, text: "ไม่สามารถบันทึกไฟล์ได้") //ไม่สามารถบันทึกไฟล์ได้
          .showAsError();
    }
  }
