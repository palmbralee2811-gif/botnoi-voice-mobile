    ...
    if (isPasswordProviderUser(user)) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          _errorMessage =
              '${'app_drawer.unable_to_delete_account_status_code'.tr()} ${response.statusCode}'; //ไม่สามารถลบบัญชีผู้ใช้ได้ รหัสสถานะ:
          _logger.e(
              "Failed to delete user account. Status Code: ${response.statusCode}");
          notifyListeners();
        }
      } catch (error) {
        _errorMessage = "${'app_drawer.error_deleting_account'.tr()} $error"; //เกิดข้อผิดพลาดในการลบบัญชี
        _logger.e("Error deleting user account: $error");
        notifyListeners();
      }
    } else {
      _errorMessage = 'app_drawer.not_logged_in_with_user_and_password'.tr(); //ไม่ได้เข้าสู่ระบบด้วยผู้ใช้และรหัสผ่าน
    ...
    if (isPasswordProviderUser(user)) {
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('app_drawer.account_deleted_successfully'.tr())), //ลบบัญชีสำเร็จ
        );
      } catch (error) {
        _errorMessage = "${'app_drawer.error_deleting_account'.tr()} $error"; //เกิดข้อผิดพลาดในการลบบัญชี
      ...
      _errorMessage = 'app_drawer.not_logged_in_with_user_and_password'.tr(); //ไม่ได้เข้าสู่ระบบด้วยผู้ใช้งานและรหัสผ่าน
