...
class EmailRegisterProvider with ChangeNotifier {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['message'] == 'success' &&
          ...
          _errorMessage = 'app_drawer.username_taken'.tr(); //ชื่อผู้ใช้งานถูกใช้ไปแล้ว
        ...
        _errorMessage = 'app_drawer.error_checking_username'.tr(); //เกิดข้อผิดพลาดในการตรวจสอบชื่อผู้ใช้
        notifyListeners();
        return false;
      }
    } catch (e) {
      _logger.e('Error during username check: $e');
      _errorMessage = 'app_drawer.error_connecting_server'.tr(); //เกิดข้อผิดพลาดในการเชื่อมต่อกับเซิร์ฟเวอร์
      notifyListeners();
      return false;
    }
  }
  ...
  Future<void> registerWithEmailPassword(
    ...
    if (password != confirmPassword) {
      _errorMessage = 'app_drawer.passwords_do_not_match'.tr(); //รหัสผ่านไม่ตรงกัน
    ...
    if (await checkUsernameAvailability(_username!)) {
      try {
        ...
        try {
          ...
          _resultMessage = "${'app_drawer.please_verify_email'.tr()} $email"; //กรุณายืนยันอีเมล:
          _errorMessage = null; // รีเซ็ต error message
          ...
          _errorMessage = 'app_drawer.unable_to_send_verification_email'.tr(); //ไม่สามารถส่งอีเมลยืนยันได้
        ...
        switch (e.code) {
          case 'email-already-in-use':
            _errorMessage = 'app_drawer.email_used_by_another_account'.tr(); //อีเมลถูกใช้โดยบัญชีอื่นแล้ว
            break;
          case 'invalid-email':
            _errorMessage = 'app_drawer.invalid_email_format'.tr(); //รูปแบบอีเมลไม่ถูกต้อง
            break;
          case 'weak-password':
            _errorMessage = 'app_drawer.password_not_strong_enough'.tr(); //รหัสผ่านไม่แข็งแรงพอ
            break;
          case 'operation-not-allowed':
            _errorMessage = 'app_drawer.action_not_allowed'.tr(); //การดำเนินการนี้ไม่ได้รับอนุญาต
            break;
          default:
            _errorMessage = 'app_drawer.error_registering'.tr(); //เกิดข้อผิดพลาดในการสมัครสมาชิก
            break;
        }
        _logger.e(
  ...
  Future<void> _registerUserToBackend(String? userId, String email) async {
    if (userId == null || _username == null) {
      ...
      _errorMessage = 'app_drawer.no_uid_or_username'.tr(); //ไม่มีรหัส UID หรือชื่อผู้ใช้งาน
      notifyListeners();
      return;
    }
    ...
    try {
      ...
      _errorMessage = 'app_drawer.error_connecting_server'.tr(); //เกิดข้อผิดพลาดในเชื่อมต่อกับเซิร์ฟเวอร์
