  ...
  Future<void> loginWithUsernamePassword(
    ...
    try {
      ...
      if (email == 'email not found') {
        _errorMessage = 'app_drawer.invalid_username'.tr(); //ชื่อผู้ใช้งานไม่ถูกต้อง
      ...
      _errorMessage = "${'app_drawer.error_logging_in'.tr()} $e"; //เกิดข้อผิดพลาดในการเข้าสู่ระบบ:
  ...
  Future<void> loginWithEmailPassword(String email, String password) async {
    try {
      ...
      if (!userCredential.user!.emailVerified) {
        _errorMessage = 'app_drawer.please_verify_email_before_login'.tr(); //กรุณายืนยันอีเมลก่อนเข้าสู่ระบบ
        _logger.w("User email is not verified: $email");

        // ส่งอีเมลยืนยันหากยังไม่ได้รับการยืนยัน
        try {
          await userCredential.user?.sendEmailVerification();
          _errorMessage = "'app_drawer.please_verify_email'.tr() $email"; //กรุณายืนยันอีเมล:
      ...
      switch (e.code) {
        case 'invalid-email':
          _errorMessage = 'app_drawer.invalid_email'.tr(); //อีเมลไม่ถูกต้อง
          break;
        case 'wrong-password':
          _errorMessage = "รหัสผ่านไม่ถูกต้อง"; //รหัสผ่านไม่ถูกต้อง
          break;
        case 'user-disabled':
          _errorMessage = "บัญชีนี้ถูกระงับการใช้งาน"; //บัญชีนี้ถูกระงับการใช้งาน
          break;
        default:
          _errorMessage = "ชื่อผู้ใช้งานหรือรหัสผ่านไม่ถูกต้อง"; //ชื่อผู้ใช้งานหรือรหัสผ่านไม่ถูกต้อง
          break;
      }
      _logger.e(
