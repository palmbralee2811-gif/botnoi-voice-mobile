...
class _ConfirmDeleteAccountScreenState
  ...
  Future<void> _deleteAccount() async {
    ...
    if (!emailProvider.isLoggedIn ||
      ...
      NotificationDialog(
        context: context,
        text: 'app_drawer.unable_to_delete_account'.tr(), //ไม่สามารถลบบัญชีได้. คุณไม่ได้เข้าสู่ระบบด้วยอีเมล
    ...
    if (!isPasswordValid) {
      NotificationDialog(
        context: context,
        text: emailDeleteAccountProvider.errorMessage ?? 'app_drawer.incorrect_password'.tr(), //รหัสผ่านไม่ถูกต้อง
      ).showErrorModal(context);
      setState(() {
        isDeleting = false;
      });
    ...
    try {
      ...
      NotificationPopup(
        context: context,
        text: '${'app_drawer.error_try_again'.tr()} $error', //เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง.
      ).showAsError();
      setState(() {
        isDeleting = false;
      });
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    GradientTextAlign(
                      'app_drawer.confirm_password'.tr(), //ยืนยันรหัสผ่าน
                    ...
                    SizedBox(height: 8.h),
                    GradientTextAlign(
                      'app_drawer.confirm_password_to_delete'.tr(), //ยืนยันรหัสผ่านของคุณเพื่อดำเนินการลบบัญชี
                      gradient: const LinearGradient(
                        colors: [
                          Color(0xFF9340FF),
                          Color(0xFF34BDFA),
